...
// Print out messages
function printMessage(zipcode, temp, location, feelsLike, relHumidity, wind, lastUpdated) {
...
function get(zipcode) {
  ...
  const request = http.get(`http://api.wunderground.com/api/${config.apiKey}/conditions/q/${zipcode}.json`, (response) => {
    let body = '';
    // Read Data
    ...
    response.on('end', () => {
      if (response.statusCode === 200) {
        try {
          ...
          // Print data
          printMessage(zipcode, weather.current_observation.temp_f,
        ...
        printError({ message: `There was an error getting the weather for ${zipcode}.
